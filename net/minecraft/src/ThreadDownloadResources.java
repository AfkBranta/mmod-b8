...
extends Thread {
    ...
    private void loadResource(File file1, String string2) {
        ...
        int i4 = 0;
        while (i4 < file3.length) {
            if (file3[i4].isDirectory()) {
                this.loadResource(file3[i4], String.valueOf(string2) + file3[i4].getName() + "/");
            } else {
                try {
                    this.mc.installResource(String.valueOf(string2) + file3[i4].getName(), file3[i4]);
                }
    ...
    private void downloadAndInstallResource(URL uRL1, String string2, long j3, int i5) {
        try {
            ...
            if (!string7.equals("sound") && !string7.equals("newsound") ? i5 != 1 : i5 != 0) {
                return;
            }
